...
```{r prep_import_and_clean, include = F, echo = F}
  ...
  feature_data = premier_data[, .(FTR, home_team_wins, away_team_wins, home_avg_goals, away_avg_goals, home_avg_sot, away_avg_sot, home_avg_corner, away_avg_corner, home_avg_foul, away_avg_foul, home_avg_yellow, away_avg_yellow, home_avg_red, away_avg_red, B365H, B365D, B365A)]
...
- OOB (out-of-bag) sample score `r 100 * round(nrow(feature_data_train[FTR != oob_pred]) / nrow(feature_data_train), 4)`% (I use this as a cross-validation score)   
- test set error of `r 100 * round(nrow(feature_data_test[FTR != test_pred]) / nrow(feature_data_test), 4)`%

There are 2 'null' models I compare against:   
* predict a home team win every time (this yields an accuracy of `r 100 * round(nrow(feature_data_test[FTR != 'h']) / nrow(feature_data_test), 4)`% on the test set)
* use betting odds features ([B365H, B365A, B365D] are the odds of a [home win, away win, draw] respectively). I take the which ever is highest as the match prediction.

So, the random forest model is slightly better than the null model. Below, I try to improve on the inital random forest model using feature selection and parameter tuning.
...
Using backwards recursive feature selection, I see that using 10 features results in a slightly more accurate model
...
The 4 features that will be removed are yellow and red cards (for both the home and away teams). Since any yellow/red card is also a foul, much of these features' information may be contained in 'home_avg_foul' and 'away_avg_foul'. UPDATE THIS

```{r parameter_tuning, echo = F}

  # Specify model found from feature selection.
  best_model = FTR ~ home_team_wins + away_team_wins + home_avg_goals + away_avg_goals + home_avg_sot + away_avg_sot + home_avg_corner + away_avg_corner + home_avg_foul + away_avg_foul

  # Set seed for reproducibility.
  set.seed(1234)

  # Create random forest.
  best_match_rf = randomForest(best_model, 
                               data = feature_data_train,
                          ntree = 250, 
                          importance = T)
