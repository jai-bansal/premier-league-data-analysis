...
I've played soccer for over a decade, so I thought it would be fun to do some analysis on professional soccer matches. I was able to find English Premier League match data from the 2003/2004 season to the 2015/2016 season (_http://www.football-data.co.uk/englandm.php_)

This document contains results, plots, and my own interpretation. The underlying code is suppressed for clarity but can be viewed in _premier_league_exploratory_analysis.Rmd_.
...
Here's a sample of the data and what all the columns stand for. This is a pretty rich data set containing not just final scores, but also some deeper metrics.   
I've removed columns that aren't used in the analysis (half time goals and results, total shots, and some betting odds).    
...
* __season_num__: added in _data_aggregation_and_cleaning.R_ to indicate which season a match occurred in
...
Below is the distribution of how many seasons the teams in our dataset have spent in the EPL.
...
So, there are many teams that break into the EPL from division beneath...and are promptly relegated the following year. Getting to the EPL is apparently on the first part of the battle.    
On the other end, there seem to be a group of dominant teams that are rarely relegated.
Looking at these clubs that played all 13 seasons in the EPL (`r seasons[seasons_played == 13]$HomeTeam`) reveals many well-known (and wealthy) teams.
...
* What features distinguish the teams that are rarely/never relegated? Are they in this data set or external?

### REFEREES:

Let's take a closer look at the people we love to hate. I'm interested in seeing summary statistics about the refs as well as whether some officials are more stringent or lenient than others. Recall that there are `r length(unique(premier_data$Referee))` referees in this dataset.
...
```
Minimum # of Matches Officiated: `r quantile(referee_only$ref_matches, 0)`
Median # of Matches Officiated: `r quantile(referee_only$ref_matches, 0.5)`
Maximum # of Matches Officiated: `r quantile(referee_only$ref_matches, 1)`
...
For the rest of this section, I'll only look at referees with 10 or more matches under their belt to avoid outlier officials with low matches. There are a few officials who only have 1 match in the data but are near the top or bottom in terms of fouls/yellow cards/red cards handed out.

Minimum Avg. Fouls Called per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_foul, 0)`
Median Avg. Fouls Called per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_foul, 0.5)`
Maximum Avg. Fouls Called per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_foul, 1)`
...
Minimum Avg. Yellow Cards Given per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_yellow, 0)`
Median Avg. Yellow Cards Given per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_yellow, 0.5)`
Maximum Avg. Yellow Cards Given per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_yellow, 1)`

This distribution is, in absolute terms, is much smaller when compared to fouls. However, considering that 2 yellow cards gets you ejected from a match, these results are more noteworthy. I find it surprising that the official at the top of the distribution is often handing out enough yellow cards to fully eject 2 players per game.

Minimum Avg. Red Cards Given per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_red, 0)`
Median Avg. Red Cards Given per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_red, 0.5)`
Maximum Avg. Red Cards Given per Match (Experienced Officials): `r quantile(exp_ref$avg_ref_red, 1)`
...
* Is there a correlation between experience and the number of fouls/yellow cards/red cards handed out?
...
* Are there any referees that just seem to have it out for a certain team?

### BEST OF THE BEST AND WORST OF THE BEST:

I want to look at the winning percentages over time of a subset of teams. In particular, I use 4 teams so the plot is readable. I also only use teams that have been in the data set for the entire 13 years. This is an important bias. Teams that have never been relegated are consistently above a certain quality. Simply put, they are the best teams. Out of these best teams, I look at the top (best of the best) and bottom (worst of the best) 2.
...
```{r best_worst, fig.width = 6, fig.height = 4.5, echo = F}
    ...
    rm(referee, referee_only, exp_ref)
...
I think it's interesting to think about what the data tells us about how different teams play. There are a variety of features relating to what could be called a team's style.
...
I'll use a random forest to predict the outcome of matches (home win, away win, draw).   
...
The _randomforest_ package provides a graphic showing which features are the most important according to 2 different metrics. It doesn't play well with RMarkdown though, so I've commented it out. Feel free to go into _premier_league_explanatory_analysis.Rmd_ to view it.
...
```{r match_outcome_prediction_initial, echo = F}
  ...
  # This doesn't load well into the RMarkdown document and so I've commented it out.
  #varImpPlot(match_rf)
...
So, the random forest model is slightly better than always predicting the home team wins. Below, I try to improve on the initial model using feature selection and parameter tuning. The training set error (0%) indicates I have an overfitting problem.
...
Backwards recursive feature selection indicates that using 12 features gives me a model that is slightly better than using the entire feature set.     
This is the feature set I'll use for parameter tuning.
...
The 2 features that are removed are _away_avg_foul_ and _home_avg_yellow_.
...
I tune 3 parameters: # of trees, # of variables considered at each split, and the minimum node size of terminal nodes.   
...
Note that I ran the parameter tuning code and noted the results, but it is now commented out but included for reference. It makes the generation of this document take way too long.
