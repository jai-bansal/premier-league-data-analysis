...
```{r clustering, echo = F}
  ...
  rm(avg_fouls_scaled, avg_shots_on_target_percent_scaled, cluster_centers, cluster_data, seasons_result, cluster_kmeans, i)
  
  # Pre-allocate features.
  # They are all NULL for now.
  premier_data$home_wins = rep(NA,                          # home team wins
                               nrow(premier_data))
  premier_data$away_wins = rep(NA,                          # away team wins
                               nrow(premier_data))
  ...
  # Add features.
  for (i in 1:max(premier_data$season_num))
    {
       
      premier_data$home_wins =  sum(premier_data[HomeTeam == season_num == i]$FTHG)
       
    }
    
    # Add 'season_wins_to_date'.
    premier_data$
