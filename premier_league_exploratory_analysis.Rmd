...
* Look for similarities among dominant teams and teams that only spent 1 season in the EPL.
...
* Redo this analysis including more teams.

### Clustering By Playing Style:

In this section, I will use K-means to cluster all `r length(unique(premier_data$HomeTeam))` teams in the data by average fouls per game and average shots on net percentage per game. The latter quantity is computed by dividing the shots on net by total shots. I only cluster by two dimensions to allow easy visualization.
